        ...
        def check():
            try:
                ...
                if not z:
                    ...
                    #label and input box
                    self.label3 = Label(self, text='Library Management system', bg='gold', font=('Georgia', 45, 'bold'))
                    self.label3.place(x=290, y=80)
                    self.label4 = Label(self, text="Enter Student Id", bg='bisque', font=('Georgia', 20, 'bold'))
                    self.label4.place(x=100, y=200)
                    self.e1 = Entry(self, textvariable=self.a, width=40).place(x=400, y=210)
                    self.srt = Button(self, text='Search', width=15, font=('Georgia', 10),command = ser).place(x=700, y=206)
                    self.label5 = Label(self, text='OR', bg='bisque', font=('Georgia', 16, 'bold')).place(x=170, y=235)
                    self.label5 = Label(self, text="Enter Book Id", bg='bisque', font=('Georgia', 20, 'bold'))
                    self.label5.place(x=100, y=260)
                    self.e2 = Entry(self, textvariable=self.b, width=40).place(x=400, y=270)
                    self.brt = Button(self, text='Find', width=15, font=('Georgia', 10),command = ent).place(x=700, y=266)
                    self.label6 = Label(self, text="Details", bg='bisque', font=('Georgia', 15, 'underline', 'bold'))
                    self.label6.place(x=20, y=350)
                    self.button = Button(self, text='Search Student', width=20, font=('Georgia', 20), command=sest).place(x=1000,y=250)
                    self.button = Button(self, text='Search Book', width=20, font=('Georgia', 20), command=sea).place(x=1000,y=350)
                    self.brt = Button(self, text="Issue Book", width=20, font=('Georgia', 20), command=ib).place(x=1000, y=450)
                    self.brt = Button(self, text="Renew Book", width=20, font=('Georgia', 20), command=rb1).place(x=1000, y=550)
                    self.brt = Button(self, text="Return Book", width=20, font=('Georgia', 20), command=ret).place(x=1000, y=650)
            except Error:
                messagebox.showerror("Error", "Something Goes Wrong")
        check()
