...
class Set(Tk):
    def __init__(self):
        ...
        def verify():
            ...
            c.set("")
            d.set("")
        ulab = Label(self, text="User Id", font=('Times New Roman', 15, 'bold')).place(x=40, y=50)
        d = StringVar()
        Uentry= Entry(self, textvariable=d, width=30).place(x=250, y=55)
        olab = Label(self,text="Old Password",font=('Times New Roman', 15, 'bold')).place(x=40,y=100)
        a=StringVar()
        entryforoldpasswd = Entry(self,show='*',textvariable=a,width = 30).place(x=250,y=105)
        label = Label(self,text="New Password",font=('Times New Roman', 15, 'bold')).place(x=40,y=150)
        b=StringVar()
        entryfornewpasswd = Entry(self,show='*',textvariable=b,width = 30).place(x=250,y=155)
        label1 = Label(self,text="Re-Type password",font=('Times New Roman', 15, 'bold')).place(x=40,y=200)
        c=StringVar()
